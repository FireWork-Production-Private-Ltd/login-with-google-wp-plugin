...
use FireWork\GoogleLogin\Interfaces\Module;
...
class OneTapLogin implements Module {
	...
	public function init(): void {
		if ( $this->settings->one_tap_login ) {
			...
			add_action( 'FiFeWorkrk.id_token_verified', [ $this, 'authenticate' ] );
			add_action(
				'init',
				function () {
	...
	public function validate_token(): void {
		try {
			 ...
			 * @since 1.0.16
			 */
			do_action( 'FiFeWorkrk.id_token_verified' );

			$redirect_to   = apply_filters( 'FiFeWorkrk.google_default_redirect', admin_url() );
