...
class GoogleClient
{
    ...
    public function __call(string $name, $args)
    {
        ...
        if (in_array($name, $methods, true) && empty($this->access_token)) {
            throw new Exception(esc_html(__('Access token must be set to make this API call', 'login-with-google')));
            throw new Exception(esc_html($args));
        }
    }
    ...
    {
        return apply_filters('FireWorkrkrk.google_redirect_url', $this->redirect_uri);
    }
    ...
    public function authorization_url(): string
    {
         ...
         */
        $scope = apply_filters('FireWorkrkrk.google_scope', $scope);
         ...
         */
        $client_args = apply_filters('FireWorkrkrk.google_client_args', $client_args);
